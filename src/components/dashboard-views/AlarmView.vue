<!-- src/components/dashboard-views/AlarmView.vue -->
<template>
  <div class="customer-view-container">
    <!-- Sidebar Filtro/Ordinamento per Clienti (non Allarmi) -->
    <FilterableSidebar
      :items="customerList"
      ...
      @item-selected="handleCustomerSelectedForAlarm" 
      @item-deleted="handleCustomerDeletedForAlarm" 
    />

    <!-- Contenuto Principale della Vista Allarmi -->
    <div class="customer-main-content">
      <div class="customer-main-header">
        <h2>Gestione Allarmi</h2>
        <div class="header-actions">
          <!-- Pulsante "Nuovo Allarme" (visibile quando nessun cliente è selezionato) -->
          <button v-if="!selectedCustomerForAlarmView" @click="addNewAlarm" class="header-action-button add-new-button">
            Nuovo Allarme
          </button>
          <!-- Pulsante "Esci" (visibile quando un cliente è selezionato) -->
          <button v-else @click="exitCustomerDetailsForAlarm" class="header-action-button exit-button">
            Esci
          </button>
        </div>
      </div>
      ...
      <div v-if="!selectedCustomerForAlarmView && !isLoading && !error && customerList.length > 0" class="no-customer-selected">
        <p>Seleziona un cliente dalla barra laterale per visualizzare e gestire i suoi allarmi.</p>
      </div>
      ...
      <!-- Contenuto quando un cliente è selezionato (per visualizzare i suoi allarmi) -->
      <div v-else-if="selectedCustomerForAlarmView" class="selected-customer-details-layout">
        <!-- Sezione Sinistra: Dettagli del Cliente Selezionato -->
        <div class="customer-data-section">
          <h3>Allarmi per {{ selectedCustomerForAlarmView.company_name }}</h3>
          <p>Qui verranno visualizzati gli allarmi specifici per il cliente selezionato.</p>
          <!-- Qui potresti aggiungere un'altra FilterableSidebar per gli allarmi di questo cliente -->
          <!-- O una tabella di allarmi -->
          <p>ID Cliente: {{ selectedCustomerForAlarmView.customer_id }}</p>
          <p>Macchine totali: {{ selectedCustomerForAlarmView.total_machines }}</p>
          <!-- Dati aggiuntivi del cliente selezionato -->
        </div>

        <!-- Sezione Destra: Pulsanti Funzione Allarme (generici o specifici per il cliente) -->
        <div class="customer-buttons-section">
          <h3>Azioni Allarmi</h3>
          <div class="button-grid">
            <button class="action-button">Visualizza Allarmi Attivi</button>
            <button class="action-button">Visualizza Storico Allarmi</button>
            <button class="action-button">Crea Nuovo Allarme</button>
            <!-- Altri pulsanti specifici per gli allarmi -->
          </div>
        </div>
      </div>
...
export default {
  ...
  setup() {
    const customerList = ref([]); // Questa lista conterrà i CLIENTI (non gli allarmi)
    const selectedCustomerForAlarmView = ref(null); // Rappresenterà il CLIENTE selezionato
    ...
    const fetchCustomers = async (pageToLoad = 1, sizeToLoad = initialPageSize, append = false, backgroundFetch = false) => {
      ...
      console.log('Headers inviati alla API /api/customer (per AlarmView sidebar):', headers);
      console.log('URL finale della richiesta (per AlarmView sidebar):', apiUrl);
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log('Dati API ricevuti per Clienti (per AlarmView sidebar):', rawData);
          if (rawData.data && rawData.data.items && rawData.data.items.length > 0) {
            console.log('Primo item API per Clienti (per AlarmView sidebar):', rawData.data.items[0]);
          }
          ...
          console.error('Errore nel caricamento Clienti (risposta non ok) per AlarmView:', response.status, errorText);
        ...
        console.error('Errore di rete o del server (catch) per Clienti in AlarmView:', err);
    ...
    const handleCustomerSelectedForAlarm = (item) => { // Rinominato per chiarezza
      selectedCustomerForAlarmView.value = JSON.parse(JSON.stringify(item));
      console.log('Cliente selezionato per visualizzare allarmi:', selectedCustomerForAlarmView.value);
      // Qui potresti voler fare una nuova chiamata API per caricare gli allarmi specifici di questo cliente
      // Es: fetchAlarmsForCustomer(selectedCustomerForAlarmView.value.customer_id);
    };
    const saveAlarmChanges = async () => { // Rinominato per chiarezza
      if (!selectedCustomerForAlarmView.value) return;
      console.log('Salvataggio modifiche per allarme (simulato):', selectedCustomerForAlarmView.value);
      alert('Logica di salvataggio allarme non implementata.');
      // Qui salveresti un allarme specifico, non un cliente
    };
    const addNewAlarm = () => { // Rinominato per chiarezza
      console.log('Aggiungi nuovo allarme cliccato.');
      // Questo potrebbe aprire un form per creare un nuovo allarme,
      // magari pre-selezionando il cliente se selectedCustomerForAlarmView.value non è null.
      selectedCustomerForAlarmView.value = { 
        customer_id: null, // ID cliente per cui si crea l'allarme
        company_name: 'Nuovo Allarme', // Titolo temporaneo
        total_machines: 0, // Gravità predefinita
        alarm_id: null, alarm_type: '', status: '', timestamp: '', machine_id: '', description: '', // Campi specifici allarme
      };
      alert('Logica per aggiungere un nuovo allarme non implementata. Form vuoto caricato.');
    };
    const exitCustomerDetailsForAlarm = () => { selectedCustomerForAlarmView.value = null; }; // Rinominato per chiarezza
    const handleCustomerDeletedForAlarm = (itemId) => { // Rinominato per chiarezza
      console.log('Richiesta di eliminazione cliente (non allarme) con ID:', itemId);
      // Questa logica elimina il cliente dalla lista, non un allarme specifico
      customerList.value = customerList.value.filter(cust => cust.customer_id !== itemId);
      if (selectedCustomerForAlarmView.value && selectedCustomerForAlarmView.value.customer_id === itemId) {
        selectedCustomerForAlarmView.value = null;
      }
      totalItems.value--;
    ...
    return {
      customerList, selectedCustomerForAlarmView, isLoading, error, hasMorePages, // selectedAlarm rinominato a selectedCustomerForAlarmView
      handleCustomerSelectedForAlarm, handleCustomerDeletedForAlarm, loadMoreCustomers,
      saveAlarmChanges, addNewAlarm, exitCustomerDetailsForAlarm // Esposti i metodi rinominati
    };
  }
};
</script>
